...
/*   Updated: 2022/10/22 17:50:49 by kyuzu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	make_images(t_data *data)
{
	...
	data->player.addr = mlx_get_data_addr(data->player.mlx_img, &data->player.bpp,
	...
	data->collectible.addr = mlx_get_data_addr(data->collectible.mlx_img,
		&data->collectible.bpp, &data->collectible.line_len, &data->collectible.endian);
	...
	while (++y < data->map->height)
	{
		...
		while (++x < data->map->width)
		{
			if (data->map->array[y][x] == WALL)
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->wall.mlx_img, x * data->block_size_x, y * data->block_size_y);
			else if (data->map->array[y][x] == COLLECTIBLE)
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->collectible.mlx_img, x * data->block_size_x, y * data->block_size_y);
			else if (data->map->array[y][x] == EXIT)
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->_exit.mlx_img, x * data->block_size_x, y * data->block_size_y);
			else
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->empty.mlx_img, x * data->block_size_x, y * data->block_size_y);
		}
	}
	return (TRUE);
}
